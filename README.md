...
```bash
python -m aletheia.interfaces.api.server --host 127.0.0.1 --port 8772 --ensure-schema
```
...
```bash
python -m aletheia.enrichment.iterative_enrichment \
...
```bash
python -m aletheia.interfaces.cli.query_metadata --all
python -m aletheia.interfaces.cli.query_artifacts
python -m aletheia.interfaces.cli.query_graph
```
