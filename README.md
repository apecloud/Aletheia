This is a professional and structured `README.md` designed for a high-impact GitHub repository. It balances technical depth with the philosophical narrative of the **Aletheia** project.
...
* **Back-test Agent**: Validates the ontology's accuracy against raw SQL queries to ensure zero data loss.
...
Ensure the generated ontology is logically sound and accurate.
```bash
# 1. Check for logical contradictions, missing links, or orphaned objects
./scripts/run_semantic_consistency.sh

# 2. Back-test the ontology by asking business questions, generating SQL, 
# and running an EXPLAIN plan against the raw MySQL DB.
./scripts/run_back_test.sh
```

### 4. Inspecting the Ontology
Once the pipeline is complete, use the unified query tool to explore the AI-generated Semantic Map stored in PostGIS.

```bash
# View everything (Tables, Columns, Objects, Links, Actions)
.venv/bin/python query_metadata.py --all

# Or query specific layers:
.venv/bin/python query_metadata.py --tables   # View tables and business comments
.venv/bin/python query_metadata.py --objects  # View AI-generated Business Objects
.venv/bin/python query_metadata.py --links    # View semantic relationships
.venv/bin/python query_metadata.py --actions  # View extracted procedures & triggers
```
