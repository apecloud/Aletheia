...
from aletheia.core.tenant_registry import TenantRegistry, default_source_db_url
from aletheia.core.text_cleaning import SearchResult, _clean_text, _is_public_web_url
...
def _candidate_identity_payload(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if item.get("element_type") in ONTOLOGY_PROPOSAL_ELEMENT_TYPES:
        ...
        operational_shape = {
            ...
            "preconditions": payload.get("preconditions") or ontology_candidate.get("preconditions"),
...
def _run_research_semantic_llm(source_text: str, frontier_item: dict[str, Any], source_ref: str) -> dict[str, Any]:
    prompt = {
        ...
        "rules": [
            ...
            "For actions, distinguish trigger_event (the state change that makes the action relevant) from preconditions (additional conditions that must still hold for the action to be valid to take right now, e.g. the target object must not already be in a terminal/handled state).",
            "For actions, express guardrails as a structured object with is_destructive, is_reversible, and requires_human_approval booleans plus a notes array for free-text caveats -- not a plain list of sentences.",
        ...
        "operational_ontology_requirements": {
            ...
            "action_shape": {
                ...
                "guardrails_shape": {
                    "is_destructive": "true if the action deletes/overwrites data or an irreversible real-world change",
                    "is_reversible": "true if the action's effects can be undone",
                    "requires_human_approval": "true if a human must confirm before/after the action runs",
                    "notes": ["free-text safety, legal, or review caveats"],
                },
                ...
                "actions": [
                    {
                        ...
                        "preconditions": ["additional conditions that must still hold for the action to be valid right now"],
                        ...
                        "guardrails": {
                            ...
                            "notes": ["approval, safety, review, or policy constraints"],
                        },
                        "evidence_quote": "exact supporting clause or sentence",
                        "confidence": 0.0,
                    }
            ...
            "ontology_candidates": [
                {
                    ...
                    "preconditions": ["for action candidates: additional conditions that must still hold for the action to be valid right now, distinct from trigger_event"],
                    ...
                    "guardrails": {
                        "is_destructive": "for action candidates: true if the action deletes/overwrites data or an irreversible real-world change",
                        "is_reversible": "for action candidates: true if the action's effects can be undone",
                        "requires_human_approval": "for action candidates: true if a human must confirm before/after the action runs",
                        "notes": ["approval/review/safety constraints when applicable"],
                    },
    ...
    if prompt_mode not in {"deep", "full"}:
        ...
        prompt["rules"] = [
            ...
            "For actions, distinguish trigger_event from preconditions, and express guardrails as {is_destructive, is_reversible, requires_human_approval, notes} rather than a plain list.",
